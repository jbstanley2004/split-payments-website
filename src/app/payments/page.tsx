...
export default function PaymentsPage() {
  ...
  const CopyBubble = ({
    ...
    <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
      <div className="max-w-md w-full rounded-2xl border border-black/70 bg-gradient-to-r from-[#ff8454] via-[#ff5400] to-[#ff8454] text-white shadow-[0_20px_45px_-25px_rgba(0,0,0,0.35)] px-6 py-5">
        <div className="space-y-2">
          <p className="text-xs font-semibold uppercase tracking-wide text-white/80">{eyebrow}</p>
          <h3 className="text-2xl md:text-3xl font-bold font-poppins leading-tight drop-shadow-[0_8px_24px_rgba(0,0,0,0.18)]">{title}</h3>
          <p className="text-sm md:text-base leading-relaxed text-white/90">{body}</p>
          <p className="text-[11px] md:text-xs font-semibold text-white/80">Hover another card to keep exploring.</p>
        </div>
      </div>
    </div>
  );
