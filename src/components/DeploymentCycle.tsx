...
import './DeploymentCycle.css';

/**
 * DeploymentCycle — user-driven, brand-aligned
 * - Start at 3 o’clock
...
export default function DeploymentCycle({ className, accent }: Props) {
  ...
  const labels = [
    { title: 'Agreement signed', angle: 0,   snap: 0.06 },
    { title: 'Merchant account activated', angle: 120, snap: 0.33 },
    { title: 'Funds deployed', angle: 240, snap: 0.66 },
  ];

  const style: React.CSSProperties = accent ? { ['--rc-accent' as any]: accent } : {};

  return (
    <div ref={ref} className={clsx('deployment-cycle', className)} style={style} data-animate="user">
      <svg className="deployment-circle" viewBox="0 0 200 200" role="img" aria-labelledby="dcTitle dcDesc">
        <title id="dcTitle">Deployment Cycle</title>
        <desc id="dcDesc">Three stages showing the deployment of merchant funding.</desc>
        ...
        <g transform="translate(100 100)">
          ...
          <g className="rc-label" textAnchor="middle">
            {labels.map((l, idx) => {
              ...
              return (
                <text key={idx} x={x.toFixed(2)} y={(y + 4).toFixed(2)} onMouseEnter={() => setByPortion(l.snap)} onMouseLeave={restoreFromScroll}>
                  {l.title}
                </text>
              );
            })}
