  ...
  return (
    <div className="pointer-events-none absolute inset-x-0 bottom-4 md:bottom-6 px-6 md:px-10 lg:px-16">
      <div className="relative mx-auto max-w-6xl overflow-hidden">
        ...
        <div className="flex items-center justify-between gap-6 rounded-2xl px-5 py-3 backdrop-blur-md backdrop-saturate-75 bg-white/60 border border-white/50 opacity-80 shadow-[0_12px_60px_-46px_rgba(0,0,0,0.32)]">
          <div className="flex flex-col gap-1">
            <OdometerText text={formattedTotal} className="text-2xl sm:text-[26px] font-semibold text-slate-800/70 font-poppins" />
            <span className="text-[9px] font-semibold uppercase tracking-[0.24em] text-slate-500/70">
              {active.totalLabel}
            </span>
          </div>

          <div className="flex flex-col items-center text-center md:items-start md:text-left gap-1">
            <span className="text-[9px] font-bold uppercase tracking-[0.24em] text-slate-500/70">
              {active.chartLabel}
            </span>
            <OdometerText text={formattedChart} className="text-lg sm:text-[19px] font-semibold text-slate-700/70 font-poppins" />
          </div>

          <div className="flex items-center gap-3 text-[11px] text-slate-500/75 font-poppins">
            <div className="hidden sm:flex items-center gap-2 min-w-[160px] justify-end text-slate-400/80">
              <CalendarClock className="h-4 w-4" />
              <span className="truncate">{active.meta}</span>
            </div>
            <div className="hidden md:block w-px h-4 bg-slate-200/70" />
            <div className="flex items-center gap-2 min-w-[150px] text-slate-400/85">
            ...
            <div className="relative flex h-2.5 w-2.5">
              <motion.span
                className="absolute inline-flex h-full w-full rounded-full bg-[#ff9161]/40"
                animate={{ scale: [1, 1.8, 1], opacity: [0.45, 0.08, 0.45] }}
                transition={{ duration: 3.2, repeat: Infinity, ease: "easeInOut" }}
              />
              <span className="relative inline-flex h-2.5 w-2.5 rounded-full bg-[#ff7a3c]/60" />
            </div>
          </div>
        </div>
      </div>
