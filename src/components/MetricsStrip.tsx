  ...
  return (
    <div className="pointer-events-none absolute inset-x-0 bottom-4 md:bottom-6 px-4 sm:px-6 md:px-10 lg:px-16">
      <div className="relative mx-auto max-w-6xl overflow-hidden">
        <div className="pointer-events-none absolute inset-y-0 left-0 w-16 bg-gradient-to-r from-white/90 via-white/55 to-transparent" />
        <div className="pointer-events-none absolute inset-y-0 right-0 w-16 bg-gradient-to-l from-white/90 via-white/55 to-transparent" />

        <div className="flex flex-wrap items-center justify-between gap-3.5 sm:gap-5 rounded-2xl px-3 sm:px-5 py-2.5 sm:py-3 backdrop-blur-lg backdrop-saturate-75 bg-white/45 border border-white/40 opacity-70 shadow-[0_14px_72px_-52px_rgba(0,0,0,0.32)] text-slate-500/65">
          <div className="flex flex-col gap-1 flex-1 min-w-[150px] basis-[48%] sm:basis-auto">
            <OdometerText text={formattedTotal} className="text-[14px] sm:text-[16px] md:text-[25px] font-semibold text-slate-500/75 font-poppins" />
            <span className="text-[7px] sm:text-[8.5px] font-semibold uppercase tracking-[0.24em] text-slate-500/60">
              {active.totalLabel}
            </span>
          </div>

          <div className="flex flex-col items-start text-left gap-1 flex-1 min-w-[150px] basis-[48%] sm:basis-auto">
            <span className="text-[7px] sm:text-[8.5px] font-bold uppercase tracking-[0.24em] text-slate-500/60">
              {active.chartLabel}
            </span>
            <OdometerText text={formattedChart} className="text-[13px] sm:text-[15px] md:text-[18px] font-semibold text-slate-500/72 font-poppins" />
          </div>

          <div className="flex items-center gap-3 text-[9px] sm:text-[10.75px] font-poppins text-slate-500/70 flex-1 min-w-[180px] basis-[48%] sm:basis-auto justify-end">
            <div className="hidden sm:flex items-center gap-2 min-w-[150px] justify-end text-slate-500/65">
              <CalendarClock className="h-3.5 w-3.5 opacity-70" />
              <span className="truncate">{active.meta}</span>
            </div>
            <div className="hidden md:block w-px h-4 bg-slate-200/60" />
            <div className="flex items-center gap-2 min-w-[140px] text-slate-500/65">
            ...
            <div className="relative flex h-2.5 w-2.5">
              <motion.span
                className="absolute inline-flex h-full w-full rounded-full bg-[#6fcf97]/28"
                animate={{ scale: [1, 1.9, 1], opacity: [0.32, 0.06, 0.32] }}
                transition={{ duration: 3.5, repeat: Infinity, ease: "easeInOut" }}
              />
              <span className="relative inline-flex h-2.5 w-2.5 rounded-full bg-[#43b37b]/45" />
            </div>
          </div>
        </div>
      </div>
