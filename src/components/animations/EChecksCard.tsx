...
export default function EChecksCard({
    ...
    return (
        <motion.div
            layout
            className={`group relative flex flex-col rounded-3xl border overflow-hidden shadow-sm transition-colors duration-500 ${isExpanded ? 'border-black' : 'bg-white border-gray-200 hover:shadow-md'}`}
            onMouseEnter={onExpand}
            initial={{ borderRadius: "1.5rem" }}
            animate={{
                backgroundColor: isExpanded ? frameTone : "#FFFFFF",
                borderColor: isExpanded ? frameTone : "#E5E7EB",
                paddingInline: isExpanded ? 0 : 12
            }}
        ...
        >
            ...
            >
                <motion.div
                    ...
                    style={{ backgroundColor: frameTone }}
                    initial={false}
                    animate={{
                        paddingLeft: isExpanded ? 0 : 20,
                        paddingRight: isExpanded ? 0 : 20,
                ...
                >
                    <motion.div
                        ...
                        animate={{
                            borderRadius: isExpanded ? 24 : 28,
                            boxShadow: isExpanded ? "none" : "0px 12px 26px rgba(0,0,0,0.12)",
                            scale: isExpanded ? 1 : 0.995
                        }}
                        transition={{ ...spring, duration: 0.6 }}
                    >
                        <div className="relative aspect-[16/9] w-full min-h-[240px] max-h-[420px]" style={{ backgroundColor: frameTone }}>
                            {videoUrl ? (
                                ...
                                <div className="absolute inset-0 w-full h-full [&>iframe]:absolute [&>iframe]:inset-0 [&>iframe]:w-full [&>iframe]:h-full">
                                    <WistiaEmbed videoId={videoId} title={videoTitle} isMuted={isMuted} />
                                </div>
                            )}
                        </div>
