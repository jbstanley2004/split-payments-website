...
type ChannelRow = {
  ...
  label: string;
  descriptor: string;
  amountK: number; // stored in thousands
  deltaPct: number; // e.g. +12.4 or -2.1
  icon?: LucideIcon;
};
...
type PeriodConfig = {
  id: PeriodId;
  label: string; // label in tabs
  chip: string; // small caption under label if needed later
  totalK: number;
  totalLabel: string;
  meta: string;
  chart: ChartConfig;
...
export default function DashboardAnimation() {
  ...
  // auto-rotate between Today → 7 days → 30 days
  useEffect(() => {
  ...
  return (
    // Let the section inherit the page's white background; card itself is dark.
    <section className="w-full">
      <div className="mx-auto flex w-full max-w-xl flex-col px-4 py-6 sm:px-0">
        {/* Card */}
        <div className="relative overflow-hidden rounded-[1.75rem] bg-slate-950 text-slate-50 p-6 shadow-[0_24px_80px_rgba(15,23,42,0.95)] md:p-7">
          {/* Gradient glows */}
          <div className="pointer-events-none absolute -right-24 -top-32 h-56 w-56 rounded-full bg-gradient-to-br from-fuchsia-500 via-rose-500 to-amber-400 opacity-80 blur-3xl" />
          <div className="pointer-events-none absolute -left-16 bottom-[-4rem] h-48 w-48 rounded-full bg-emerald-500/40 opacity-70 blur-3xl" />
          ...
          <div className="relative z-10 flex flex-col gap-6">
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="flex items-center gap-3">
                <div className="flex h-9 w-9 items-center justify-center rounded-2xl bg-black/40 shadow-inner">
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
              <div>
                ...
                <p className="mt-4 text-3xl font-semibold tracking-tight text-slate-50 sm:text-4xl">
                  {formattedTotal}
                </p>
              </div>
              ...
              <div className="space-y-2.5">
                {active.channels.map((row) => {
                  ...
                  return (
                    <div
                      key={row.id}
                      className="flex items-center justify-between gap-3 rounded-2xl bg-white/5 px-3 py-2.5 backdrop-blur-sm"
                    >
                      <div className="flex items-center gap-3">
                        {Icon && (
                          <span className="inline-flex h-7 w-7 items-center justify-center rounded-xl bg-black/40">
