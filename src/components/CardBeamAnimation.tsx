...
class CardStreamController {
  ...
  setupEventListeners() {
    ...
    this.cardLine.addEventListener("wheel", (e) => {
      e.preventDefault();
  ...
  generateCode(width: number, height: number): string {
    ...
    while (flow.length < totalChars + width) {
      const extra = library[randInt(0, library.length - 1)].replace(/\s+/g, " ").trim();
      flow += " " + extra;
    }
...
export default function CardBeamAnimation() {
  ...
  return (
    <div className="card-beam-container">
      <div className="card-beam-wrapper">
