    ...
    >
          ...
          {visual ? (
            ...
            <>
              <div className="dark:hidden">
                <Image src={imageSrcLight || '/graphic4.png'} alt="Split merchant dashboard showing sales based funding and payment analytics" width={600} height={380} priority className="drop-shadow-[0_40px_60px_rgba(0,0,0,0.15)] rounded-xl w-full h-auto" />
              </div>
              <div className="hidden dark:block">
                <Image src={imageSrcDark || '/graphic4.png'} alt="Split merchant dashboard dark mode view" width={600} height={380} priority className="drop-shadow-[0_40px_60px_rgba(0,0,0,0.3)] rounded-xl w-full h-auto" />
              </div>
            </>
          )}
        </motion.div>
      ...
      <div className="max-w-xl relative z-10 text-center lg:text-left w-full lg:w-1/2">
        ...
        <div className="mt-8 flex flex-wrap gap-4 justify-center lg:justify-start">
          ...
          <a href="/#funding" className="text-[var(--theme-text-primary)] font-lora hover:text-[var(--theme-accent)] transition-colors duration-300 text-base inline-flex items-center">
            Learn more →
          </a>
        </div>
      </div>
...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    const initCarousel = () => {
      ...
      function makeBoxes(n:number){ for (let i=0;i<n;i++){ const b:any=document.createElement('div'); boxes.push(b); c.appendChild(b);} }
      makeBoxes(30);
      // match original settings
      (window as any).gsap.to(c, 0.4, { perspective: 200, backgroundColor: 'transparent' });
      for (let i=0;i<boxes.length;i++){
        const b:any = boxes[i];
        (window as any).gsap.set(b,{ left:'50%', top:'50%', margin:-200, width:400, height:400, borderRadius:'20%', backgroundImage:'url(' + industryImages[i] + ')', backgroundSize:'cover', backgroundPosition:'center', clearProps:'transform', backfaceVisibility:'hidden' });
        b.tl=(window as any).gsap.timeline({ paused:true, defaults:{ immediateRender:true }})
          .fromTo(b,{ scale:0.31, rotationX:i/boxes.length*360, transformOrigin:String('50% 50% -800%') },{ rotationX:'+=360', ease:'none'})
          .timeScale(0.05);
      }
      function animate(){ if(!isDragging){ currentRotation += autoRotateSpeed; if(Math.abs(velocity)>0.00001){ currentRotation += velocity; velocity *= 0.95; } } currentRotation = currentRotation % 1; if(currentRotation<0) currentRotation += 1; boxes.forEach((b:any)=>{ (window as any).gsap.set(b.tl,{ progress: currentRotation });}); requestAnimationFrame(animate);} animate();
      const startDrag=(y:number)=>{ isDragging=true; lastY=y; lastTime=Date.now(); velocity=0; (c as any).style.cursor='grabbing'; if (typeof navigator!== 'undefined' && 'vibrate' in navigator) navigator.vibrate(15); };
      const onDrag=(y:number)=>{ if(!isDragging) return; const t=Date.now(); const dt=Math.max(t-lastTime,1); const dY=y-lastY; const d = -dY*0.002; currentRotation += d; velocity = -dY*0.002/(dt/16.67); lastY=y; lastTime=t; };
      const endDrag=()=>{ if(!isDragging) return; isDragging=false; (c as any).style.cursor='grab'; if (typeof navigator!== 'undefined' && 'vibrate' in navigator) navigator.vibrate([12,10,8]); };
      c.addEventListener('mousedown',(e:any)=>{ e.preventDefault(); startDrag(e.clientY); });
      document.addEventListener('mousemove',(e:any)=> onDrag(e.clientY));
      document.addEventListener('mouseup', endDrag);
      c.addEventListener('touchstart',(e:any)=>{ e.preventDefault(); startDrag(e.touches[0].clientY); },{ passive:false });
      document.addEventListener('touchmove',(e:any)=> onDrag(e.touches[0].clientY));
      document.addEventListener('touchend', endDrag);
      (c as any).style.cursor='grab';
    };
    const checkGSAP = setInterval(() => { if ((window as any).gsap) { clearInterval(checkGSAP); initCarousel(); } }, 100);
  ...
  return (
    <>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-start">
          ...
          <div className="order-1 lg:order-2 space-y-6">
            <div>
              ...
              <p className="font-lora">As you process with us, we review your rhythm and extend additional rounds of funding on a cadence that fits your business — biweekly, monthly, or as needed. The goal is to grow together, not to run a single transaction.</p>
            </div>
            <div>
              <h4 className="font-poppins text-lg font-semibold">Why it works</h4>
              <p className="font-lora">You already process card payments. Repayments come from a fixed percentage of those sales, so payments move in step with your revenue. When you have a slow day, you pay less. If there are no sales, there’s no payment.</p>
            </div>
            <div>
              <h4 className="font-poppins text-lg font-semibold">What we focus on</h4>
              <p className="font-lora">Our attention is on your card processing revenue. As long as that flow stays consistent, we keep funding you in new rounds. Typical qualification is around eight thousand dollars per month in processing volume. We don’t base decisions on personal credit or bank balances.</p>
            </div>
            <div>
      ...
      {/* Section 2 – carousel stays untouched */}
      <section
