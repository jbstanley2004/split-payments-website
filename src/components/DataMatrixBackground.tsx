...
export default function DataMatrixBackground() {
  ...
  return (
    <div className="pointer-events-none absolute inset-0 -z-10 overflow-hidden" aria-hidden>
      ...
      <div className="relative h-full w-full">
        {items.map((item, index) => {
          ...
          return (
            <motion.div
              key={`${item}-${index}`}
              className="absolute text-[8px] sm:text-[9px] md:text-[10px] font-semibold tracking-[0.18em] uppercase text-slate-900/16 blur-[1.25px] select-none"
              style={{ top: position.top, left: position.left }}
              animate={{
                x: ["0%", `${driftAmount}%`, "0%", `-${driftAmount / 2}%`, "0%"],
                y: ["0%", `-${driftAmount / 3}%`, "0%", `${driftAmount / 5}%`, "0%"],
                opacity: [0.12, 0.16, 0.14, 0.15, 0.12]
              }}
      ...
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_center,rgba(255,255,255,0.55)_0%,rgba(255,255,255,0.85)_50%,rgba(255,255,255,0.98)_78%)]" />
    </div>
  );
}
