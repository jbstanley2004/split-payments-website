...
// Design tokens for the new, flatter visual language
const COLORS = {
  background: "#F5F5F3",
  text: "#161616",
  muted: "#555555",
...
function AutoQualificationBlock() {
  return (
    <section className="mb-12 border-b border-black/5 pb-10">
      <header className="mb-6 max-w-2xl">
        <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-black/60">
          Eligibility
        </p>
        <h3 className="mt-2 text-2xl md:text-3xl font-poppins font-semibold text-[#161616]">
        ...
        <p className="mt-3 text-sm md:text-base text-[#555]">
          ...
          <span className="font-semibold text-[#161616]"> If you’re processing with us and your numbers qualify, we’ll tap you on the shoulder.</span>
        </p>
      </header>

      <div className="flex flex-wrap gap-3 text-xs md:text-sm text-[#333]">
        {"Based on real sales, not projections|No separate underwriting package to compile|Offers scale up as your processing grows"
          .split("|")
          .map((item) => (
            <div
              key={item}
              className="inline-flex items-center gap-2 rounded-full border border-black/10 px-3 py-1"
            >
  ...
  return (
    <section className="mb-14">
      <div className="mb-6 flex flex-col gap-3">
        <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-black/60">Timeline</p>
        <h3 className="text-xl md:text-2xl font-poppins font-semibold text-[#161616]">Funding in days, not months</h3>
        <p className="max-w-2xl text-sm md:text-base text-[#555]">
      ...
      <div className="relative">
        <div className="pointer-events-none absolute inset-y-0 left-[14px] hidden w-px bg-black/10 md:block" />
        <div className="space-y-4 md:space-y-5">
          {timelineSteps.map((step, index) => (
            <motion.div
              ...
              className="relative flex flex-col rounded-xl border border-black/8 bg-white px-5 py-4 md:flex-row md:items-center md:gap-6 md:px-6"
            >
              <div className="mb-3 flex items-center gap-3 md:mb-0 md:w-40">
                ...
                <span className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[#161616]">
                  {step.label}
                </span>
              </div>

              <div className="flex-1">
                <h4 className="mb-1.5 text-sm md:text-base font-poppins font-semibold text-[#161616]">
                  {step.title}
                </h4>
                <p className="text-sm text-[#444]">{step.body}</p>
              </div>
            </motion.div>
          ))}
        </div>
  ...
  return (
    <section className="mb-14">
      <div className="flex flex-col items-center text-center">
        <h3 className="text-lg md:text-xl font-poppins font-semibold text-[#161616]">
          A funding loop that matches your sales cycle
        </h3>
        <p className="mt-3 max-w-xl text-sm md:text-base text-[#555]">
      ...
      <div className="mt-10 flex flex-col items-center gap-8 lg:flex-row lg:items-stretch lg:gap-14">
        <div className="relative flex h-[320px] w-[320px] items-center justify-center rounded-full border border-black/10 bg-white sm:h-[360px] sm:w-[360px] lg:shrink-0">
          <div className="relative z-10 flex h-32 w-32 flex-col items-center justify-center rounded-full border border-black/10 bg-white px-4 text-center">
            <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-black/60">Loop</p>
            <p className="mt-1 text-xs font-poppins font-semibold text-[#161616]">Deploy → Repay → Renew</p>
          </div>

          <div className="absolute inset-6 rounded-full border border-dashed border-black/15" />

          <div className="pointer-events-none absolute inset-6">
            <svg viewBox="0 0 200 200" className="h-full w-full" aria-hidden="true">
              <circle cx="100" cy="100" r={ringRadius} stroke="#E0E0DD" strokeWidth={2} fill="none" />
            </svg>
          </div>
          ...
          >
            {fundingStages.map((stage, index) => {
              ...
              return (
                ...
                >
                  <motion.div
                    className="inline-flex items-center gap-1.5 rounded-full border border-black/10 bg-white px-3 py-1.5"
                    style={{ rotate: inverseRotation }}
                    animate={{
                      borderColor: isActive ? COLORS.accent : "rgba(0,0,0,0.08)",
                  ...
                  >
                    <span className="inline-block h-1.5 w-1.5 rounded-full bg-[#FF4306]" />
                    <span className="text-[11px] md:text-xs">{stage.label}</span>
                  </motion.div>
                </div>
              );
            })}
        ...
        <div className="max-w-md space-y-4 text-sm md:text-base text-[#555]">
          <div className="border border-black/10 bg-white px-4 py-3 rounded-xl">
            <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-black/60">Current step</p>
            <p className="mt-1 text-sm font-poppins font-semibold text-[#161616]">{activeStage.label}</p>
            <p className="mt-1 text-xs md:text-sm">{activeStage.description}</p>
          </div>

          <p>
            Instead of fixed monthly payments,
            <span className="font-semibold text-[#161616]"> repayment flexes with your sales.</span> On strong days, you pay down more. On slower days, less. There’s no penalty for early payoff and no compounding interest.
          </p>
          <p>
            ...
            <span className="font-semibold text-[#161616]"> keep investing in growth without reapplying.</span>
          </p>
          <div className="mt-3 grid gap-3 text-xs md:text-sm">
            <div className="flex items-start gap-2">
              <span className="mt-1 inline-block h-1.5 w-1.5 rounded-full bg-[#FF4306]" />
              <p>
                <span className="font-semibold text-[#161616]">No compounding interest or variable rates.</span> Just a simple, transparent cost of capital tied to your sales volume.
              </p>
            </div>
            <div className="flex items-start gap-2">
              <span className="mt-1 inline-block h-1.5 w-1.5 rounded-full bg-[#FF4306]" />
              <p>
                <span className="font-semibold text-[#161616]">Works for seasonal and growth-stage businesses.</span> Repayment naturally slows during off-peak periods.
              </p>
            </div>
          </div>
        </div>
...
function ReassuranceStrip() {
  return (
    <section className="mt-4 border-t border-black/5 pt-8">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div className="space-y-1">
          <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-black/60">
            Built for established card volume
          </p>
          <p className="max-w-xl text-sm md:text-base text-[#555]">
        ...
        <div className="flex flex-wrap items-center gap-3 text-xs md:text-sm text-[#333]">
          <div className="inline-flex items-center gap-2 rounded-full border border-black/10 px-3 py-1">
            <span className="inline-block h-1.5 w-1.5 rounded-full bg-[#FF4306]" />
    ...
    <section className="py-16 md:py-20 bg-transparent">
      <div className="mx-auto max-w-4xl px-6 sm:px-8">
        <header className="mb-10">
          <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-black/60">Funding</p>
          <h2 className="mt-1 text-3xl sm:text-4xl font-poppins font-semibold tracking-tight text-[#161616]">
            How funding works
          </h2>
          <p className="mt-4 max-w-2xl text-sm font-lora text-[#555] sm:text-base md:text-lg">
